...
> Concurrency is the execution of multiple instruction sequences at the same time. Traditionally, the Operating System scheduler will rapidly context switch between threads so all make progress. In Go, this is managed by the runtime with a [cooperative scheduler](https://en.wikipedia.org/wiki/Cooperative_multitasking).
